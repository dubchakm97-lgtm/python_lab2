...
import shutil
import shlex
from src.constants import SHELL_FILE, TRASH_DIR
...
def undo() -> None:
    ...
    if not control_str:
        ...
        log_message("No rm/mv/cp commands to undo", logging.WARNING)
        return
    ...
    if cmd == "rm":
        try:
            ...
            log_message(f"Undo rm {name} -> {dst_original}", logging.INFO)
            ...
            log_message(f"undo rm — file not found in trash ({src_in_trash})", logging.WARNING)

        except Exception as e:
            print(f"Ошибка при откате rm: {e}")
            log_message(f"undo rm failed ({e})", logging.ERROR)
        ...
        try:
            ...
            log_message(f"Undo mv {dst} -> {src}", logging.INFO)
            print(f"Объект возвращён: {dst} → {src}")

        except FileNotFoundError:
            print(f"Не удалось откатить mv: {real_dst} не найден")
            log_message(f"undo mv — destination not found ({real_dst})", logging.WARNING)

        except Exception as e:
            print(f"Ошибка при откате mv: {e}")
            log_message(f"undo mv failed ({e})", logging.ERROR)
        ...
        try:
            ...
            if not target.exists():
                ...
                log_message(f"undo cp — target not exists ({target})", logging.WARNING)
                return
            ...
            log_message(f"Undo cp remove {target}", logging.INFO)
            print(f"Копия {target} удалена (откат cp).")

        except Exception as e:
            log_message(f"undo cp failed ({e})", logging.ERROR)
