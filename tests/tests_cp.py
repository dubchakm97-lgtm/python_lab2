...
class TestCp:
    def test_cp_file_to_file(self):
        ...
        with patch.object(mod_cp.pathlib, "Path", side_effect=[src, dst]), \
             patch.object(mod_cp.shutil, "copy2") as m_copy2, patch("builtins.print") as m_print:
    ...
    def test_cp_dir_with_r(self):
        ...
        dst = Mock(); dst.expanduser.return_value = dst
        with patch.object(mod_cp.pathlib, "Path", side_effect=[src, dst]), \
             patch.object(mod_cp.shutil, "copytree") as m_copy2, patch("builtins.print") as m_print:
